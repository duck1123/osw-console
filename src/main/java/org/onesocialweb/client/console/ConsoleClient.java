...
public class ConsoleClient implements InboxEventHandler {
    ...
    private void processArgs(String args[])
    {
        ...
        if (hasPort) {
            ...
            doCommand(extractCmd("/connect "+args[0]),
                      extractArgs("/connect "+args[0]  + " " + args[2] ));
        }
        else
        ...
        switch(args.length)
        {
            ...
            case 3:
                if (!hasPort)
                    doCommand(extractCmd("/login "+args[1] + " " +args[2] ),
                              extractArgs("/login "+args[1]+ " " +args[2] )); break;
            case 4:
                doCommand(extractCmd("/login "+args[3]), extractArgs("/login "+args[3] )); break;
            case 5:
                doCommand(extractCmd("/login "+args[3] + " " +args[4] ),
                          extractArgs("/login "+args[3]+ " " +args[4] )); break;
        }
    }
    ...
    private void upload()
        throws ConnectionRequired, AuthenticationRequired, IOException {
        try {
    ...
    private void activities(String jid)
        throws ConnectionRequired, AuthenticationRequired {
        try {
    ...
    private void relations(String jid)
        throws ConnectionRequired, AuthenticationRequired {
        try {
    ...
    private void subscribe(String user)
        throws ConnectionRequired, AuthenticationRequired {
        try {
    ...
    private void unsubscribe(String user)
        throws ConnectionRequired, AuthenticationRequired {
        try {
    ...
    private void subscriptions(String jid)
        throws ConnectionRequired, AuthenticationRequired {
        try {
    ...
    private void subscribers(String jid)
        throws ConnectionRequired, AuthenticationRequired {
        try {
    ...
    private void addRelation()
        throws IOException, AuthenticationRequired, ConnectionRequired {
    ...
    private void updateRelation(String id)
        throws IOException, AuthenticationRequired, ConnectionRequired {
        final String prompt = reader.getDefaultPrompt();
    ...
    private void setProfileKey(String key)
        throws ConnectionRequired, AuthenticationRequired, IOException {
        if (profile == null) {
    ...
    private void clear(String key)
        throws ConnectionRequired, AuthenticationRequired {
        if (profile != null) {
            for (Iterator<Field> i = profile.getFields().iterator();
                 i.hasNext();) {
                Field field = i.next();
    ...
    private void shout(String recipient)
        throws ConnectionRequired, AuthenticationRequired, IOException {
    ...
    public void privacy() throws IOException {
        ...
        if (mode.equalsIgnoreCase("e")) {
            action = aclFactory.aclAction(AclAction.ACTION_VIEW,
    ...
    private void badArgs(String cmd) {
        error("incorrect number of arguments to " + cmd +
              ", expecting: /" + cmd + " " + findCommand(cmd).args);
    }
    ...
    private String render(ActivityEntry activity) {
        ...
        String result = published + " \t| " +
            (author != null ? author + " \t| " : "") + status + "\n";
        return result;
    }

    private String render(Relation relation) {

        String published = (relation.hasPublished()) ?
            DateFormat.getDateTimeInstance(DateFormat.SHORT,
                                           DateFormat.SHORT).format(relation.getPublished()) :
            null;
        ...
        String result = (published != null ? published + " \t| " : "") +
            (origin != null ? origin + " \t| " : "") +
            (target != null ? target + " \t| " : "") + "\n" +
    ...
    private void delete(String actNr)
        throws ConnectionRequired, AuthenticationRequired
    {
    ...
    private void updateActivity(String actNr)
        throws ConnectionRequired, AuthenticationRequired
    {
    ...
    private void commentActivity(String actNr)
        throws ConnectionRequired, AuthenticationRequired
    {
    ...
    private void queryReplies(String actNr)
        throws ConnectionRequired, AuthenticationRequired
    {
        int intActNr=Integer.parseInt(actNr);
        List<ActivityEntry> activities=inbox.getEntries();
